...
POST /api/v1/cloud/load_balancers/with/delete_protection/list
...
云上把实例规格拆为 `exclusive`（是否独占型）与 `sla_type`（性能容量型档位）两个正交字段，前端合成一列「实例规格」展示，筛选条件对应关系如下：

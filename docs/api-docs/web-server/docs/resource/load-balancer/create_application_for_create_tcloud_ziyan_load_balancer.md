...
- 该接口提供版本：v1.7.0.0+。
- 该接口所需权限：负载均衡创建。
- 该接口功能描述：业务下创建自研云负载均衡申请。

### URL

POST /api/v1/cloud/vendors/tcloud-ziyan/applications/types/create_load_balancer
...
| name                       | string       | 是  | 名称                                                            |
| zones                      | string array | 否  | 主可用区, 【自研云】支持填对内网型填多个，匹配多可用区                                  |
...
| memo                       | string       | 否  | 备注                                                            |
| zhi_tong                   | bool         | 否  | 【自研云】是否开启直通特性                                                 |
| tgw_group_name             | string       | 否  | 【自研云】指定Tgw独占集群标签, 指定"ziyan_mianliu"时开启免流特性                    |

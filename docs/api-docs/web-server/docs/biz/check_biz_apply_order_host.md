...
- 该接口提供版本：v9.9.9+。
- 该接口所需权限：业务访问。
- 该接口功能描述：校验业务下主机申领时继承套餐的主机信息。

### URL

POST /api/v1/woa/bizs/{bk_biz_id}/task/check/apply/order/host

### 输入参数

| 参数名称       | 参数类型 | 必选 | 描述                           |
|---------------|--------|------|-------------------------------|
| require_type  | int	 | 是	| 需求类型(3: 机房裁撤 6:滚服项目)  |
| bk_asset_id   | string | 是   | 主机固资号                      |
| region        | string | 是   | 机器所在的地域                   |

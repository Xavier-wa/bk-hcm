...
description: "[bkdevbuddy] 代码评审入口 — 拉取本次变更，委派 code-reviewer 对照项目准则库评审，产出分级报告并回写 MR/PR"
---
...
- **判断依据只有项目准则库**，不是你的通用经验。库里没有的问题最多以 Info 建议 + 准则提议的形式出现。

> 术语分层：**准则**是评审期的经验性判据（可豁免、可被驳回、按采纳率淘汰）；**rule** 是编码期的强制约束，由准则晋升而来；**红线 / 检查器**是机器可判定的最终形态。三者不要混用，见 `docs/naming-convention.md`。

## 前置检查（第一步，不可跳过）

调 `bkdevbuddy_code_review_criteria`，它返回准则库是否已建、准则 id/标题清单，以及**待决的准则提议**：

- `initialized: false` → 停下来告诉用户先跑 `bkdevbuddy code-review init`（别名 `cr init`），说明准则库是评审依据，缺它只能凭通用经验猜。用户坚持要审再继续，但要提前说明本次只会产出 Info 建议。
- 已建但 `count: 0` → 正常，继续。这是新库的预期状态，头几次评审的主要产出是准则提议。

拿到的**待决提议清单要带给子代理**：同一个想法如果上次已经提过，让它复用原 id 而不是另起一个。id 漂移会让"这条被提了 4 次还没人采纳"这种信号散成四条各提一次，看不出任何东西。

你自己**不需要**通读准则正文——`code-reviewer` 子代理会在自己的上下文里读 `criteria.md`。主对话只要 id 清单，避免把准则库全文塞进主线上下文。
...
- **能读工作区文件**——它要自己读准则库与改动，纯对话形态不行；
- **能力完整**——若该 IDE 的只读子代理形态拿不到必要工具，选能力完整的那种，用下面第 5 条的 prompt 约束来兜住"只读"。

当前 IDE 没有子代理能力时，降级在主对话内评审，但**先告知用户代价**：准则库与完整 diff 会留在主线上下文里，后续对话都要带着它们。

prompt 里必须包含：

1. 要求先读 `<dataDir>/know-how/agents/code-reviewer.md` 并严格遵循；
2. 完整 diff（含文件路径与改动行号集合）；
3. 准则库路径 `<dataDir>/code-review/criteria.md`，让它自己读；
4. 上一步拿到的待决提议清单（id + 标题），要求同类想法复用已有 id；
5. **硬性约束：只读不写**——禁止使用任何编辑工具、禁止"顺手修一下"，唯一产物是评审结论。

为什么放进子代理：准则库全文 + 完整 diff 是一大坨输入，评审完就没用了。放子代理的独立上下文里消化，主对话只接一份结构化 findings，随后的报告与回写阶段不被污染。

子代理回传：findings 列表（每条含 `criterion` / `severity` / `file` / `line` / `evidence` / `message` / `fix` / `confidence`）、准则提议列表、本次豁免清单。
...
| 检查 | 处理 |
| --- | --- |
| `severity` 是 Block/Warn 但 `criterion` 为空或不在库中 | **强制降为 Info** |
| finding 落在未改动行 | 丢弃；除非 `evidence` 说明了本次改动导致其失效的因果（移动文件、改 alias、组件挪进私有目录等） |
| `evidence` 是转述而非原文片段 | 回退让它补原文，或降为 Info |
| 豁免注释 `// cr-ignore <criterionId> -- <理由>` 缺理由 | 该豁免无效，finding 照常报出，并单独指出理由缺失 |
...
[Info] 未命中任何准则 — 建议追加新准则？
  这里对同一份列表数据做了三次 filter，可提取为 computed。
  ↳ 提议追加准则「重复遍历应合并为单次 computed」
    id: duplicate-list-filter
    回复「采纳 duplicate-list-filter」写入准则库

本次跳过 1 处豁免：
  form-item-consistency @ front/src/.../form.vue:42（理由：三方组件无法改 props）
```

三个要点：Block 必然带准则 id 与修复方案；无出处的判断只能是 Info；新发现以"提议"形态出现等人决定。

**豁免必须出现在报告里**。不写出来它就会退化成一个静默关闭检查的开关。同一准则的豁免理由反复出现，说明准则本身定宽了或前提有误，提示用户复审该条。
...
```
context: <评审标识>
hits:    [每条 finding 命中的准则 id，命中几处就重复几次]
waived:  [本次被豁免跳过的准则 id]
proposals: [本轮提出的准则提议]
```
...
无准则出处的 Info **不记**（它没有 id 可记）；这一步只记有 id 的东西。
...
bkdevbuddy code-review add --id <id> --title <标题> \
  --requirement <要求> --why <理由> --bad <反例> --good <正例>
```

写入后该提议会自动从待决列表清除，之后由这条准则自己的触发计数接手。

**不要**自行写入准则库，也不要复用或改写已有**准则** id——id 是 finding 与 stats 的连接键，改动会切断历史数据。用户没表态就留在报告里，不追问第二次；提议会留在待决列表，下次评审再提到时累加次数。
...
3. 用**复合键** `<criterion>|<file>|<归一化 evidence>` 逐条匹配（归一化 = 去首尾空白、压缩连续空格；**不含行号**，因为改动会让行号漂移）；
4. 判定每条：
...
```
context:  <与首轮相同的评审标识>
accepted: [判定为已修复的准则 id]
rejected: [被驳回的准则 id]
hits:     [本轮新引入问题命中的准则 id]
```

**"未修复"三者都不记**。它既没被采纳也没被驳回，只是还没处理完；把它算进任何一边都会污染采纳率。

**驳回即数据**。开发者认为某条是误报，直接在 MR 评论里回一句就行，对账时记为 rejected，不需要任何额外操作。采纳率（1 − 误报率）是后续判断某条准则该晋升还是该删的唯一客观依据。

## 定期回看

用户问"哪些准则该晋升 / 该删"时（月度量级），跑：
...
- **采纳率的分母是已判定数，不是触发数**。还没复审的既不算采纳也不算驳回，所以新准则显示 `--` 是正常的，不是没人理它。
- **晋升看的是不同 context 数，不是 hits**。同一个 MR 里命中 7 次只算一次；7 次不同评审各命中一次才说明这是普遍问题。
- **建议是建议**。引擎判断不了某条准则是否机器可判定，晋升成 rule 还是固化成 eslint 规则得人来定。

## 硬约束

- 评审过程中**不得修改任何业务代码**，包括"顺手改个 typo"。
- 报告未经用户确认**不得**回写到平台。
- 准则提议未经用户明确采纳**不得**写入准则库。
- 不代替用户合并 MR/PR。

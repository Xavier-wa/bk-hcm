...
description: HCM 前端项目专家助手 — 了解项目架构、组件库、编码规范
---
...
> 本项目的架构、禁用模式（红线）、bkui-vue3 查阅约定都已沉淀到自动生效的 rules（`.cursor/rules/` 下的 `fe-menu-route-architecture` / `fe-deprecated` / `fe-auth-migration` / `fe-router-action` / `fe-bkui-usage` / `fe-conventions` 等），编码时务必遵循。

...
> 这份模板在 know-how 同步后位于 `<projectRoot>/.bkdevbuddy/know-how/skills/test-checklist/assets/`，可直接读出来作为初始内容。
...
4. **回写结论**
   ...
   - 失败用例需要在 coding 阶段修复后重测，可由用户决定是否回退到 coding 阶段（`bkdevbuddy wf set coding`）

5. **推进**
   - 验证结论补齐后，明确询问用户是否确认本次测试结果可作为发布 / 合入依据
   - 用户确认后调用 `bkdevbuddy_workflow_approve`（stage=test），再 `bkdevbuddy_workflow_next` 进入 done
   - 被拒就按 `reasons` 补齐再试

...
本文件服务 form / detail / search / table-column 四类字段模型。`@Column` 是共享定义入口，但各用途消费的元数据不同：

| 用途 | 重点属性 | 说明 |
|------|----------|------|
| form | `required`、`rules`、`apiOnly`、`meta.display` | 驱动输入组件、校验和提交数据 |
| detail | `group`、`meta.display.render` | 驱动分组与只读展示；通常不配置校验 |
| search | `name`、`option`、`meta.display` | 驱动筛选条件及选择器 |
| table-column | `name`、`index`、`sort`、`option` | 驱动列顺序、排序与值映射 |

不要为了复用而让单个模型同时承载四类用途的全部字段；字段集合或元数据明显不同时，按用途拆分类，并共享基础字段。

## @Column 参数详解

字段通过 `@Column(type, options)` 定义，常用 `options` 如下：
...
| `group` | `string` | 详情字段分组 | `'基本信息'` |
| `index` | `number` | 表格列顺序 | `0` |
| `sort` | `boolean` | 表格列是否可排序 | `true` |
| `meta.display.appearance` | `string` | 组件外观类型 | `'radio'` |
| `meta.display.props` | `Record<string, any>` | 组件额外 props | `{ type: 'textarea', rows: 3 }` |
| `meta.display.render` | `Function` | 详情或表格自定义渲染 | `(value) => value` |

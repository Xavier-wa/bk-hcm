...
末尾的 HTML 注释是**复审对账的锚点**，必须带上，格式 `<!-- cr-fp: <criterion>|<file>|<归一化 evidence> -->`。归一化 = 去首尾空白、连续空格压成一个。它**不含行号**——改动会让行号漂移，带行号的键在复审时必然对不上。
...
拿不准就当作**未驳回**，并在对账表里标出"回复语义不明确，请确认"。把有歧义的回复算成驳回，会让误报率数据虚高，进而误导后续该不该删掉某条准则。

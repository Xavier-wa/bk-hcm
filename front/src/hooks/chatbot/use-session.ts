...
import { isCardActionProtected } from './card-action-protect';
import { captureCardClientMeta, type CardClientMeta } from './card-client-meta';
import type { ChatSession } from './types';

export type RefreshSessionOpts = {
  /** passive：获焦/可见性/跨签广播；active：用户显式打开面板、续跑前校验等（不受保护窗口 skip） */
...
export function useSession(deps: SessionDeps) {
  ...
  // 守卫：无选中会话 / 正在流式输出 / 空会话 → 跳过；passive + 卡片操作保护窗口 → 跳过（防获焦闪卡）。
  // 不预清空 messages，也不切 isLoadingHistory：旧内容保持可见；SNAPSHOT 内原子合并 clientMeta。
  const refreshCurrentSession = async (opts: RefreshSessionOpts = {}) => {

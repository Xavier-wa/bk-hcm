...
const bizRef = useTemplateRef<any>('bizRef');
const typeRef = useTemplateRef<any>('typeRef');
const offsetRef = useTemplateRef<any>('offsetRef');
const memoRef = useTemplateRef<any>('memoRef');
...
const handleSelectClear = (key: keyof IQuotaOffset) => {
  // 使用类型断言允许表单字段设置为 undefined
  (localData.value as Record<string, any>)[key] = undefined;
};
  ...
  <tr>
    ...
    <td>
      <SelectColumn
        ...
        @clear="handleSelectClear('type')"
      />
    </td>
    <td>
      <InputColumn
        ref="offsetRef"
        v-model="localData.offset"
        type="number"
        :min="0"
        :rules="[

...
```
/hcm/create-branch <新分支名> <目标分支名>
```

示例：
```
/hcm/create-branch feat-my-feature origin/bcc/v1.8.x
```
...
- **无参数**：使用交互式对话收集以下信息：
  ...
  - **基础分支**（选择）：要基于的远程分支名，提供常用选项：
    - `origin/bcc/v1.8.x`（BCC 1.8.x 版本）
    - `origin/github/dev`（开发分支）
    - `origin/main`（主分支）
    - 其他（让用户输入自定义分支名）
  - **是否启动开发服务器**（选择）：创建完成后是否启动 `npm run dev:bcc`
...
```
/hcm/create-branch feat-ticket-new-feature origin/bcc/v1.8.x
```

# crp-device-type-sync Specification

## Purpose

定义 CRP 机型族映射数据同步到本地 `woa_device_type_physical_rel` 表的能力规范。系统通过周期性拉取 CRP `queryCvmTypeList` 接口数据，采用两段式 diff（仅新增 + 更新，不删除）将虚拟机机型与物理机机型族的映射关系同步到本地表，作为主机回收链路（短租到期、用户退机、主动退机申请）查询机型族信息的权威本地数据源，使业务读路径与 CRP 状态完全解耦。

...
定义 GraphAgent 的场景分发节点 `scene_dispatch`：作为 Graph 入口与单一路由决策中心，依据会话级 `StateKeySessionTag` 与本轮 `StateKeyIntent` 集中决定下一跳（直达 `llm`、进入 `intent_recognition`、或经 `fallback` 给出未支持提示并回流），不调用 LLM。

# Spec: agent-metrics-export

## Purpose

定义 agent-server 的指标导出能力。通过 OpenTelemetry Prometheus Exporter 将 trpc-agent-go 框架内置的 OTel metrics（Chat / Tool / Agent 三大类共 13 个指标）桥接到现有 Prometheus Registry，并通过 `/metrics` 端点暴露 Token 消耗、LLM 延迟、工具执行时间等核心运营指标，便于监控告警与容量规划。

...
系统 SHALL 通过写入 etcd reset flag（复用 watcher 的 lease）通知 watch 循环执行重置，不直接覆盖 cursor key，避免与在途 watch 轮次的 cursor 提交产生竞态。生效方式为 watch 循环每轮迭代开头消费 reset flag 并将 cursor 置空，MUST NOT 要求重启 master，也不对 watch goroutine 做通知/中断；正在执行中的当轮消费不被打断。

#### Scenario: 下一轮生效

- **WHEN** 重置接口成功写入 reset flag 到 etcd
- **THEN** 对应 watch 流下一轮循环消费 flag、将 cursor 置空并从当前时间起消费，全程无需重启

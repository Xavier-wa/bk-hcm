# bill-adjustment-res-sub-class

## Purpose

调账明细的资源类别细分能力：资源类别 `res_class` 由原 `cpu`/`gpu` 两值扩展为 `cpu`、`gpu_card`、`gpu_api`、`gpu_other` 四值，并新增单一字段 `res_sub_class` 承载卡型或模型厂商。该字段的必填性与取值域由资源类别与记录所属云厂商共同决定，贯通创建、更新、列表查询与导出全链路，并配套存量 `gpu` 数据的一次性刷新。

# bill-adjustment-sub-class-enum-api

## Purpose

调账资源子类的枚举查询能力：按云厂商分别提供卡型清单与模型厂商清单两个查询接口，供前端渲染 `res_sub_class` 的下拉选项。清单来源以账单上报侧的既有实现与 `global_config` 运营配置为准，并与创建、更新接口的取值域校验同源，以排除「下拉能选、提交被拒」的不一致。

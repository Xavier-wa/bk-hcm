# load-test-subnet-config Specification

## Purpose

提供压测子网配置的管理能力：运营管理员可通过 woa-server 全量维护 region→vpc_id→[subnet_id] 三层映射，使用方可读取该映射。配置统一存储于 global_config 表（config_type=cvm_apply，config_key=load_test_subnet）。

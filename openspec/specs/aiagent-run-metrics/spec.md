# Spec: aiagent-run-metrics

## Purpose

定义 AI Agent 一轮对话的 Prometheus 指标：在 `pkg/metrics` 注册 `hcm_aiagent` 子系统（无业务开关），事件回调就地打点且不等写库，cancel / unknown / 耗时直方图各自的计数边界。该能力不包含 `aiagent_run` 表结构或 data-service 接口。
...
- `hcm_aiagent_run_duration_seconds`（Histogram）：label `state`（仅 `finished` / `error`）、`bkcc_biz_id`、`scene`
...
- **THEN** `run_total{state="finished",bkcc_biz_id=<本轮>,scene=<本轮 session_tag>}` MUST +1，耗时直方图 MUST 记一笔 `state="finished"` 且带同一 `bkcc_biz_id`，`inflight` MUST -1

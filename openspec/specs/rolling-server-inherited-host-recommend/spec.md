# rolling-server-inherited-host-recommend Specification

## Purpose

为滚服项目（`require_type=6`）的"以旧换新"提单场景提供继承固资推荐能力：在 woa-server 滚服前缀下提供业务视角与资源视角两条接口，按用户选定的地域与机型族实时查询 CMDB，返回按已使用时长排序的可继承固资候选（每族最多 5 条），并标记计费满 36 个月的推荐项，帮助用户在申领滚服机器时快速选出待置换的旧固资。

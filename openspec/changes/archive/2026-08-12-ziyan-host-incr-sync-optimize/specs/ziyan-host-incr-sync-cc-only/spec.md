...
hc-service SHALL 提供新的增量同步入口（`SyncHostWithCCByCond`），接收 cloud-server watch 增量事件下发的 hostIDs（单批 ≤100，含 AccountID/BizID），对每台主机以 CC 当前态与 DB 存量比对（bk_host_id + cloud_id 双维度）确定处理类别，MUST NOT 以事件类型或单看 DB 作为分类依据。

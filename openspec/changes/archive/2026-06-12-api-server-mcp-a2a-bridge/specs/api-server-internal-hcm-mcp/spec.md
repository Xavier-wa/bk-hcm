...
### Requirement: 工具集从本地 OpenAPI yaml 加载

api-server SHALL 在启动阶段从 `ApiServerSetting.MCP.Internal.OpenAPISpecPath` 指向的本地 OpenAPI 3.0 yaml 文件解析工具集，并 SHALL 注册到 `trpc-mcp-go.Server` 实例。
...
api-server SHALL 支持配置 `ApiServerSetting.MCP.Internal.IncludeOperationIDs` 白名单：
...
- **WHEN** api-server 启动且 `mcp.internal.enable: true`、`openapiSpecPath` 指向合法 yaml
- **THEN** 南向 MCP `tools/list` SHALL 在启动后立即返回 yaml 中所有命中白名单的工具
...
- **WHEN** `mcp.internal.enable: true` 但 `openapiSpecPath` 指向的文件不存在
...
- **WHEN** yaml 含 203 个 operationId，`IncludeOperationIDs: ["list_cvms", "list_*_quota"]`
...
- **WHEN** `IncludeOperationIDs` 为空数组或缺失
- **THEN** 南向 MCP `tools/list` SHALL 包含 yaml 中**全部**有合法 `operationId` 的接口

### Requirement: 内部专用工具（OpenAPI yaml 之外的本地工具）

api-server SHALL 支持配置 `ApiServerSetting.MCP.Internal.InternalOnlyTools` 列表，定义**网关资源未发布、由 api-server 本地代码维护**的内部专用工具。

...
### Requirement: 裁撤项目固定配置

**Reason**: 裁撤项目由动态 `dissolve_project` 配置提供，不再写死在服务配置文件。

**Migration**: 删除 `resourceDissolve.projectIDs`，改在 `global_config` 的 `dissolve_project` 配置维护项目 ID；`listExcludedProjectNames`(`[]string`) 改名为 `listExcludedProjectIDs`(`[]int`)。

...
同步任务 SHALL 在写库前补全主机全部字段：裁撤系统直接提供的 `project_id`、`asset_id`、`inner_ip`、`module`、`abolish_phase`、`project_name`、`cpu_core`（取 `cpuLogicCoreNum`）直接落库；`region` SHALL 由设备 `availabilityZoneName` 匹配 `woa_zone.zone_name` 得到对应 `region_id`；`bk_biz_id`、`group_id`、`operators` SHALL 按「CC 优先、ES 兜底」规则补全。
...
同步任务 SHALL 以 `(project_id, asset_id)` 复合键比对裁撤系统数据与 DB 现有数据，得出新增/更新/删除集合，允许同一 `asset_id` 在不同项目并存；MUST 移除按 `asset_id` 去重择优的逻辑。

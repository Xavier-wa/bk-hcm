...
func validHostType(collection string, projection map[string]int, result interface{}, rid interface{}) error {
	if result == nil {
		logs.Errorf("host query result is nil, rid: %s", rid)
	...
	if resType.Kind() != reflect.Ptr {
		logs.Errorf("host query result type(%v) not pointer type, rid: %v", resType, rid)
		return fmt.Errorf("host query result type invalid")
	}
	...
	if resType.ConvertibleTo(reflect.TypeOf(&map[string]interface{}{})) {
		if resType != reflect.TypeOf(&metadata.HostMapStr{}) {
			logs.Errorf("host query result type(%v) not match *metadata.HostMapStr type, rid: %v", resType, rid)
			return fmt.Errorf("host query result type invalid")
		}
		return nil
	}
	...
	case reflect.Struct:
		...
		for i := 0; i < numField; i++ {
			...
			if bsonTag == "" {
				logs.Errorf("host query result field(%s) has empty bson tag, rid: %v", field.Name, rid)
				return fmt.Errorf("host query result type invalid")
			}
			if hostSpecialFieldMap[bsonTag] && field.Type != validType {
				logs.Errorf("host query result field type(%v) not match *metadata.StringArrayToString type", field.Type)
				return fmt.Errorf("host query result type invalid")
			}
		}
	case reflect.Slice:
		...
		if elem.ConvertibleTo(reflect.TypeOf(map[string]interface{}{})) {
			if elem != reflect.TypeOf(metadata.HostMapStr{}) {
				logs.Errorf("host query result type(%v) not match *[]metadata.HostMapStr type", resType)
				return fmt.Errorf("host query result type invalid")
			}
			return nil
		}
		...
		if elem.Kind() != reflect.Struct {
			logs.Errorf("host query result type(%v) not struct pointer type or map type", resType)
			return fmt.Errorf("host query result type invalid")
		}
	...
	default:
		logs.Errorf("host query result type(%v) not pointer of map, struct or slice, rid: %v", resType, rid)
		return fmt.Errorf("host query result type invalid")
	}
	return nil
}

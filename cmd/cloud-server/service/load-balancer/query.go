...
func (svc *lbSvc) listTargetsHealthByTGID(cts *rest.Contexts, validHandler handler.ValidWithAuthHandler) (
	...
	case enumor.TCloudZiyan:
		return svc.getTCloudTargetHealth(cts.Kit, tgID, req,
			svc.client.HCService().TCloudZiyan.Clb.ListTargetHealth)
	default:
...
func (svc *lbSvc) getListenerDetailByIDAndBiz(kt *kit.Kit, vendor enumor.Vendor, bizID int64, lblID string) (
	*corelb.TCloudListener, *types.CloudResourceBasicInfo, error) {
	...
	case enumor.TCloud:
		...
		lblInfo = &lblResp.Details[0]
	case enumor.TCloudZiyan:
		lblResp, err := svc.client.DataService().TCloudZiyan.LoadBalancer.ListListener(kt, listReq)
		if err != nil {
			logs.Errorf("fail to list tcloud ziyan listener(%s), err: %v, rid: %s", lblID, err, kt.Rid)
			return nil, nil, err
		}
		if len(lblResp.Details) == 0 {
			return nil, nil, errf.Newf(errf.RecordNotFound, "listener not found, id: %s", lblID)
...
func (svc *lbSvc) getListenerByCloudID(kt *kit.Kit, cloudID string) (*corelb.TCloudListener, error) {
	...
	case enumor.TCloudZiyan:
		listenerDetail, err := svc.client.DataService().TCloudZiyan.LoadBalancer.GetListener(kt, lblInfo.ID)
		if err != nil {
			logs.Errorf("get tcloud ziyan listener detail failed, lblID: %s, cloudID: %s, err: %v, rid: %s",
				lblInfo.ID, cloudID, err, kt.Rid)
			return nil, err
		}
		return listenerDetail, nil

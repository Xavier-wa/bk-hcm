...
// AuthorizeByUsers 内部版鉴权接口扩展方法（自研云单据审批场景使用），测试用桩实现固定返回空结果。
func (f *fakeAuthorizer) AuthorizeByUsers(_ *kit.Kit, _ []string, _ ...meta.ResourceAttribute) (
...
// TestListBizExclusiveClusterIdleVips_Success 正常查询：归属校验通过后按账号所属云厂商（TCloud）调用对应
// hc-service 拿到闲置 VIP 列表。
func TestListBizExclusiveClusterIdleVips_Success(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch {
		case strings.Contains(r.URL.Path, "idle_vips/describe"):
			writeOKRespCS(t, w, map[string]any{"count": 2, "details": []string{"1.1.1.1", "1.1.1.2"}})
		case strings.Contains(r.URL.Path, "resources/basics"):
			// 账号基础信息查询：用于确定云厂商，决定分发到 TCloud/TCloudZiyan 哪个 hc-service 客户端
			writeOKRespCS(t, w, map[string]any{"vendor": "tcloud"})
		default:
			writeOKRespCS(t, w, map[string]any{
				"count":   1,

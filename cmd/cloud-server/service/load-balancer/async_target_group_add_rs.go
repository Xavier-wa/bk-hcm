...
func (svc *lbSvc) batchAddBizTarget(cts *rest.Contexts, authHandler handler.ValidWithAuthHandler) (any, error) {
	...
	case enumor.TCloud:
		return svc.buildAddTCloudTarget(cts.Kit, bizID, req.Data, accountInfo)
...
func (svc *lbSvc) buildAddTCloudTarget(kt *kit.Kit, bkBizID int64, body json.RawMessage,
	accountInfo *types.CloudResourceBasicInfo) (interface{}, error) {
	...
	// 检查传入的Target是否已存在
	if err := svc.checkTargetExists(kt, req, accountInfo.AccountID); err != nil {
		return nil, err
	}
	...
	for _, group := range req.TargetGroups {
		...
		if len(relList) == 0 {
			rsIDs, err := svc.batchCreateTargetDb(kt, group.Targets, tgId, accountInfo.AccountID)
			if err != nil {
	...
	return svc.buildAddTCloudTargetTasks(kt, bkBizID, accountInfo, lbIDs[0], targetGroupRsListMap)
}
...
func (svc *lbSvc) buildAddTCloudTargetTasks(kt *kit.Kit, bkBizID int64, accountInfo *types.CloudResourceBasicInfo,
	lbID string, tgMap map[string][]*dataproto.TargetBaseReq) (interface{}, error) {
...
func (svc *lbSvc) initFlowAddTargetByLbID(kt *kit.Kit, accountInfo *types.CloudResourceBasicInfo,
	...
	if err = svc.buildSubFlow(kt, flowID, lbID, converter.MapKeyToSlice(tgMap), enumor.TargetGroupCloudResType,
...
func (svc *lbSvc) createTargetGroupAddRsTaskDetails(kt *kit.Kit, taskManagementID string, bkBizID int64,
	...
	details, err := svc.createTaskDetails(kt, taskManagementID, bkBizID,
		enumor.TaskTargetGroupAddRS, details)
	if err != nil {

...
func (a *accountSvc) GetAccount(cts *rest.Contexts) (interface{}, error) {
	...
	case enumor.Ziyan:
		return a.getTCloudZiyanAccount(cts, accountID)
	case enumor.Other:
...
func (a *accountSvc) getTCloudZiyanAccount(cts *rest.Contexts, accountID string) (interface{}, error) {
	acc, err := a.client.DataService().TCloudZiyan.Account.Get(cts.Kit, accountID)
	if err != nil {
		logs.Errorf("get tcloud_ziyan acc failed, err: %v, rid: %s", err, cts.Kit.Rid)
		return nil, err
	}
	// 敏感信息不显示，置空
	if acc != nil {

...
// BatchListenerModifyRsWeightExecutor 监听器批量调整RS权重执行器
...
type batchListenerModifyRsWeightTaskDetail struct {
	taskDetailID string
	flowID       string
	actionID     string
...
// Execute 导入执行器的唯一入口
func (c *BatchListenerModifyRsWeightExecutor) Execute(kt *kit.Kit, source enumor.TaskManagementSource,
	...
	err = c.validate(kt)
	if err != nil {
		logs.Errorf("validate listener modify rs weight failed, err: %v, source: %s, rid: %s", err, source, kt.Rid)
		return "", err
	}
...
// Run 执行器执行入口
func (c *BatchListenerModifyRsWeightExecutor) Run(kt *kit.Kit, source enumor.TaskManagementSource) (string, error) {
	// 创建异步管理任务、任务详情列表
...
func (c *BatchListenerModifyRsWeightExecutor) unmarshalData(rawDetail json.RawMessage) error {
...
func (c *BatchListenerModifyRsWeightExecutor) validate(kt *kit.Kit) error {
...
func (c *BatchListenerModifyRsWeightExecutor) filter() {
	return
}
...
func (c *BatchListenerModifyRsWeightExecutor) createTaskManagement(
	kt *kit.Kit, source enumor.TaskManagementSource) (string, error) {
...
func (c *BatchListenerModifyRsWeightExecutor) createFlowTask(kt *kit.Kit, lbID string,
	...
	if err != nil {
		logs.Errorf("call taskserver to batch listener modify rs weight custom flow failed, err: %v, lbID: %s, "+
...
func (c *BatchListenerModifyRsWeightExecutor) updateTaskManagementAndDetails(kt *kit.Kit,
...
func (c *BatchListenerModifyRsWeightExecutor) updateTaskManagement(kt *kit.Kit, taskID string, flowIDs []string) error {
...
func (c *BatchListenerModifyRsWeightExecutor) updateTaskDetailsState(kt *kit.Kit, state enumor.TaskDetailState,
	taskDetails []*batchListenerModifyRsWeightTaskDetail) error {

...
正确的调用顺序（每次使用 Skill 都必须严格遵守）：
1. **第一步 — 只调用 `skill_load`**：该步只发出 `skill_load` 这一个工具调用，不要同时发出任何其他工具调用，也不要生成面向用户的文本回复。
2. **第二步 — 收到 `skill_load` 返回后，立即调用 `skill_run`**：阅读加载结果中的 SKILL.md 内容，理解该 Skill 的可用命令和参数格式，然后**直接发出 `skill_run` 工具调用**。不要停下来给用户发文本消息，不要解释"我已加载技能"之类的话，直接调用。

⚠️ 关键约束：
- **`skill_load` 和 `skill_run` 不能在同一批次工具调用中发出**（系统架构限制：同批次中 `skill_load` 的状态变更不会立即生效，`skill_run` 会因此失败）。
- **`skill_load` 返回后必须立即继续调用 `skill_run`**，不要生成纯文本回复。如果你在 `skill_load` 之后只输出文本而不调用 `skill_run`，系统会认为你已完成任务并终止流程，用户将无法获得结果。

⚠️ **禁止使用交互式 Skill 工具**：
- 不要调用 `skill_exec`、`skill_poll_session`、`skill_write_stdin`、`skill_kill_session`。
- 这些工具用于 stdin/TTY 交互式会话，在当前 AGUI 环境下不适用。
- **始终使用 `skill_run`**（一次性执行）来运行 Skill 命令。

...
func TestIsToolConfirmInterruptKey(t *testing.T) {
	tests := []struct {
		...
		{key: constant.ToolConfirmCreateCvmApplyInterruptKey, want: true},
...
func TestBuildInterruptCustomEvent(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			meta: graph.PregelStepMetadata{
				NodeID:         string(enumor.SubgraphAgentNodeHostApply),
				InterruptKey:   interruptKey,
				InterruptValue: payload,
			},
			wantName: "",
		},
		{
			...
			meta: graph.PregelStepMetadata{
				NodeID:       string(enumor.MainGraphAgentNodeFallback),
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			customEvt, ok := buildInterruptCustomEvent(context.Background(), pregelInterruptEvent(tc.meta))
			if tc.wantName == "" {
				if ok || customEvt != nil {

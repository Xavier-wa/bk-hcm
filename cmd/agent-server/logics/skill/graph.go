...
func MakeSkillLoadAfterToolCallback(agentName string) graph.AfterNodeCallback {
	return func(ctx context.Context, callbackCtx *graph.NodeCallbackContext, state graph.State, result any,
		...
		if nodeErr != nil {
			logs.Warnf("[skill_graph] nodeErr != nil, skip: %v", nodeErr)
			return result, nil
		}
		...
		if !ok || len(msgs) == 0 {
			logs.Warnf("[skill_graph] no messages in state (ok=%v len=%d), skip skill load tracking", ok, len(msgs))
			return result, nil
		}

		assistantMsg := extractLastAssistantWithToolCalls(msgs)
		if assistantMsg == nil {
			logs.Infof("[skill_graph] no assistant message with tool calls found, skip skill load")
			return result, nil
		}
		...
		for _, tc := range assistantMsg.ToolCalls {
			logs.Infof("[skill_graph] processing tool call: name=%s", tc.Function.Name)
			switch tc.Function.Name {
			case constant.SkillLoadToolName:
				recordSkillLoadedToState(tc, agentName, inv.Session.State)
			case constant.SkillSelectDocsToolName:
				recordSkillSelectedDocsToState(tc, agentName, inv.Session.State)
			}
		}

		logs.Infof("[skill_graph] AfterNodeCallback completed, node=%s", callbackCtx.NodeName)
		return result, nil
	}
}
...
func recordSkillLoadedToState(tc model.ToolCall, agentName string, state map[string][]byte) {
	var params struct {
		Skill string `json:"skill"`
	}
	if err := json.Unmarshal(tc.Function.Arguments, &params); err != nil {
		logs.Warnf("[skill_graph] failed to unmarshal skill_load args: %v", err)
		return
	}
	if params.Skill == "" {
		return
	...
	logs.Infof("[skill_graph] skill loaded: agent=%s skill=%s", agentName, params.Skill)
}

// recordSkillSelectedDocsToState skill_select_docs 调用后将 skill 的选中的 docs 写入 session.State
func recordSkillSelectedDocsToState(tc model.ToolCall, agentName string, state map[string][]byte) {
	var params struct {
		Skill          string   `json:"skill"`
	...
	if err := json.Unmarshal(tc.Function.Arguments, &params); err != nil {
		logs.Warnf("[skill_graph] failed to unmarshal skill_select_docs args: %v", err)
		return
	}
	if params.Skill == "" {
		return
	...
	if params.IncludeAllDocs {
		...
		if err != nil {
			logs.Warnf("[skill_graph] failed to marshal docs: %v", err)
			return
		}
		state[docsKey] = docsBytes
	}
	logs.Infof("[skill_graph] skill docs selected: agent=%s skill=%s all=%v docs=%v",
		agentName, params.Skill, params.IncludeAllDocs, params.Docs)
}

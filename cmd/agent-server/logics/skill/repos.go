...
func NewSkillRepos(repo skillpkg.Repository, scenes ...enumor.IntentType) (*SkillRepos, error) {
	if repo == nil {
		logs.Errorf("[new skill repos] repository is nil")
		return nil, fmt.Errorf("[new skill repos] repository is nil")
	}
	m := make(map[enumor.IntentType]skillpkg.Repository, len(scenes))
	for _, s := range scenes {
		m[s] = repo
	}
	return &SkillRepos{Scene: m}, nil
}

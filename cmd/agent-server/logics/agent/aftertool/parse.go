...
func parseItems(result string, rid string) []*woaserver.ApplyRecommendItem {
	...
	if err := json.Unmarshal(data, &resp); err != nil {
		logs.Errorf("parse items failed, failed to unmarshal data: %v, rid: %s", err, rid)
		return nil
	}
	return resp.Items
}
...
func parseSuborders(result string, rid string) []*woaserver.ApplyRecommendSuborder {
	data := extractData([]byte(result), 0)
	if len(data) == 0 {
		return nil
	}
	...
	if err := json.Unmarshal(data, &resp); err != nil {
		logs.Errorf("parse suborders failed, failed to unmarshal data: %v, rid: %s", err, rid)
		return nil
	}
...
func extractData(content []byte, depth int) []byte {
	if depth > constant.ExtractDataMaxDepth || len(content) == 0 {
		return content
	}

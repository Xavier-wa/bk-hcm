	...
	err := concurrence.BaseExec(constant.SyncConcurrencyDefaultMaxLimit, params.CloudIDs, func(param string) error {
		...
		if syncResult, err = cli.securityGroupRule(kt, syncOpt); err != nil {
			logs.ErrorDepthf(1, "[%s] account: %s sg: %s sync sgRule failed, err: %v, rid: %s",
				enumor.TCloudZiyan, params.AccountID, param, err, kt.Rid)
			return err
		}
...
func (cli *client) securityGroupRule(kt *kit.Kit, opt *syncSGRuleOption) (*SyncResult, error) {
	...
	sg, err := cli.dbCli.TCloudZiyan.SecurityGroup.GetSecurityGroup(kt, opt.SGID)
	if err != nil {
		logs.Errorf("[%s] request dataservice get tcloud ziyan security group failed, err: %v, rid: %s",
	...
	for _, one := range rulesFromDB {
		...
		default:
			logs.Errorf("[%s] unknown security group rule type: %s, skip handle, rid: %s", enumor.TCloudZiyan,
				one.Type, kt.Rid)
			continue
		}
...
// listSGRuleFromCloud list tcloud ziyan security group rule from database
func (cli *client) listSGRuleFromDB(kt *kit.Kit, sgID string) (
	...
	for {
		...
		listResp, err := cli.dbCli.TCloudZiyan.SecurityGroup.ListSecurityGroupRule(kt, listReq, sgID)
		if err != nil {
			return nil, err
		}
...
// listSGRuleFromCloud list tcloud ziyan security group rule from cloud
func (cli *client) listSGRuleFromCloud(kt *kit.Kit, region, cloudSGID string) (string,
	...
	if err != nil {
		logs.Errorf("[%s] request adaptor to list tcloud ziyan security group rule failed, err: %v, rid: %s",
			enumor.TCloudZiyan,
			err, kt.Rid)
...
func (cli *client) deleteSGRule(kt *kit.Kit, sgID string, delIDs []string) error {
	...
	for batchIdx, delIdBatch := range delIdBatches {
		...
		err := cli.dbCli.TCloudZiyan.SecurityGroup.BatchDeleteSecurityGroupRule(kt, req, sgID)
		if err != nil {
			logs.Errorf("[%s] request dataservice to delete tcloud ziyan security group rule failed,"+
				" err: %v,batch idx：%d, rid: %s", enumor.TCloudZiyan, err, batchIdx, kt.Rid)
			return err
		}
	}
...
func (cli *client) createSGRule(kt *kit.Kit, sgID string, allRules []corecloud.
	...
	for batchIdx, ruleBatch := range splitRuleBatches {
		...
		resultBatch, err := cli.dbCli.TCloudZiyan.SecurityGroup.BatchCreateSecurityGroupRule(kt, req, sgID)
		if err != nil {
			logs.Errorf("[%s] request dataservice to create tcloud ziyan security group rule failed, "+
				"err: %v,batch idx: %d, rid: %s", enumor.TCloudZiyan, err, batchIdx, kt.Rid)
			return nil, err
		}

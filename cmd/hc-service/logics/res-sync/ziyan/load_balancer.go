	...
	for {
		...
		if err != nil {
			logs.Errorf("[%s] request dataservice to list lb failed, err: %v, req: %v, rid: %s",
				enumor.TCloudZiyan, err, req, kt.Rid)
			return err
		}
...
func (cli *client) createLoadBalancer(kt *kit.Kit, accountID string, region string,
	...
	if _, err := cli.dbCli.TCloudZiyan.LoadBalancer.BatchCreateClb(kt, &lbCreateReq); err != nil {
		logs.Errorf("[%s] call data service to create load balancer failed, err: %v, rid: %s",
...
func (cli *client) deleteLoadBalancer(kt *kit.Kit, accountID string, region string, delCloudIDs []string) error {
	...
	if len(delLBFromCloud) > 0 {
		logs.Errorf("[%s] lb not exist before sync deletion, opt: %v, failed_count: %d, rid: %s",
			enumor.TCloudZiyan, checkParams, len(delLBFromCloud), kt.Rid)
	...
	logs.Infof("[%s] sync to delete lb success, accountID: %s, count: %d, rid: %s",
		enumor.TCloudZiyan, accountID, len(delCloudIDs), kt.Rid)
...
func (cli *client) listLBFromDB(kt *kit.Kit, params *SyncBaseParams) ([]corelb.TCloudLoadBalancer, error) {
	...
	result, err := cli.dbCli.TCloudZiyan.LoadBalancer.ListLoadBalancer(kt, req)
	if err != nil {
...
func convCloudToDBCreate(cloud typeslb.TCloudClb, accountID string, region string, vpcMap map[string]*common.VpcDB,
	...
	lb.Extension = convertZiyanExtension(cloud, region, clusterMap)

	return lb
}

// convertZiyanExtension 转换云上负载均衡数据为本地扩展字段。clusterMap 为按云上独占集群 ID 批量查询本地独占
// 集群表得到的 cloud_id -> 本地记录映射，用于填充独占集群 clusters 回显信息。
func convertZiyanExtension(cloud typeslb.TCloudClb, region string,
	...
	// 内网独占集群（VPCGW），与下方公网独占集群（ClusterTag/ClusterIds）无关
	...
	// 公网独占集群信息：数据来源于 ClusterTag（7层独占标签）与 ClusterIds（集群ID数组，可能混合TGW/STGW落地ID）这两个
...
func convCloudToDBUpdate(id string, cloud typeslb.TCloudClb, vpcMap map[string]*common.VpcDB,
	...
	lb := protocloud.LoadBalancerExtUpdateReq[corelb.TCloudClbExtension]{
		...
		CloudSubnetID:    cloudSubnetID,
		Tags:             cloud.GetTagMap(),
		Extension:        convertZiyanExtension(cloud, region, clusterMap),
		Isp:              cvt.PtrToVal(cloud.VipIsp),
	}
	if cloud.NetworkAttributes != nil {
		lb.BandWidth = cvt.PtrToVal(cloud.NetworkAttributes.InternetMaxBandwidthOut)

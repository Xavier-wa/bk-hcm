...
func (cli *client) createListener(kt *kit.Kit, accountID, region string, syncOpt *SyncListenerOption,
	...
	if len(dbListeners) > 0 {
		lblCreated, err := cli.dbCli.TCloudZiyan.LoadBalancer.BatchCreateListener(kt,
	...
	if len(dbRules) > 0 {
		ruleCreated, err := cli.dbCli.TCloudZiyan.LoadBalancer.BatchCreateTCloudListenerWithRule(kt,
			&dataproto.ListenerWithRuleBatchCreateReq{ListenerWithRules: dbRules})
		if err != nil {
			logs.Errorf("fail to create ziyan listener with rule while sync, err: %v syncOpt: %+v, rid: %s",
				err, syncOpt, kt.Rid)
			return nil, err
		}
...
func (cli *client) updateListener(kt *kit.Kit, bizID int64, region string,
	...
	err := cli.dbCli.TCloudZiyan.LoadBalancer.BatchUpdateTCloudListener(kt,

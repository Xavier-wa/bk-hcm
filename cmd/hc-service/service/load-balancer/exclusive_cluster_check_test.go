...
type fakeExclusiveAdaptor struct {
	...
	bwPkgErr         error
	clusterResources *tclb.DescribeClusterResourcesResponseParams
	clusterErr       error
}
...
func (f *fakeExclusiveAdaptor) DescribeClusterResources(_ *kit.Kit,
	_ *typelb.TCloudDescribeClusterResourcesOption) (*tclb.DescribeClusterResourcesResponseParams, error) {
...
func TestRecheckExclusiveBeforeDeliver_VipNoLongerIdle(t *testing.T) {
	...
	adaptor := &fakeExclusiveAdaptor{
		clusterResources: &tclb.DescribeClusterResourcesResponseParams{
			ClusterResourceSet: []*tclb.ClusterResource{
				{Vip: cvt.ValToPtr("1.1.1.1"), Idle: cvt.ValToPtr("False")},
			},
		},
	}

...
func (svc *service) SyncCvmCCInfoByCond(cts *rest.Contexts) (interface{}, error) {
	...
	if err != nil {
		logs.Errorf("list aws cvm failed, err: %v, req: %v, rid: %s", err, converter.PtrToVal(listReq), cts.Kit.Rid)
		return nil, err
	}
...
func (h *cvmCCInfoSyncHandler) nextCvms(kt *kit.Kit) ([]cvm.BaseCvm, error) {
	...
	if err != nil {
		logs.Errorf("list aws cvm failed, err: %v, account id: %s, region: %s, rid: %s", err, h.accountID, h.region,
			kt.Rid)
		return nil, err
	}
...
func (h *cvmCCInfoSyncHandler) syncCCInfo(kt *kit.Kit, cvms []cvm.BaseCvm) error {
	params := &aws.SyncCvmCCInfoParams{
		Cvms: cvms,
	}

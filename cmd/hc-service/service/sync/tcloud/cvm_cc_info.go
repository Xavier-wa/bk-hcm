...
func (svc *service) SyncCvmCCInfo(cts *rest.Contexts) (interface{}, error) {
	...
	syncCli, err := svc.syncCli.TCloud(kt, req.AccountID)
	if err != nil {
		logs.Errorf("init tcloud sync client failed for account %s, err: %v, rid: %s", req.AccountID, err, cts.Kit.Rid)
		return nil, err
	}
...
func (svc *service) SyncCvmCCInfoByCond(cts *rest.Contexts) (interface{}, error) {
	...
	if err != nil {
		logs.Errorf("list tcloud cvm failed, err: %v, req: %v, rid: %s", err, converter.PtrToVal(listReq), cts.Kit.Rid)
		return nil, err
	}

	syncCli, err := svc.syncCli.TCloud(cts.Kit, req.AccountID)
	if err != nil {
		return nil, err
	}
...
func (h *cvmCCInfoSyncHandler) nextCvms(kt *kit.Kit) ([]cvm.BaseCvm, error) {
	...
	if err != nil {
		logs.Errorf("list tcloud cvm failed, err: %v, account id: %s, region: %s, rid: %s", err, h.accountID, h.region,
			kt.Rid)
		return nil, err
	}
...
func (h *cvmCCInfoSyncHandler) syncCCInfo(kt *kit.Kit, cvms []cvm.BaseCvm) error {
	params := &tcloud.SyncCvmCCInfoParams{
		Cvms: cvms,
	}

...
func (hd *imageHandler) Next(kt *kit.Kit) ([]string, error) {
	...
	if err != nil {
		logs.Errorf("request adaptor list tcloud ziyan image failed, err: %v, opt: %v, rid: %s", err, listOpt, kt.Rid)
		return nil, err
	}
...
func (hd *imageHandler) Sync(kt *kit.Kit, cloudIDs []string) error {
	...
	if _, err := hd.syncCli.Image(kt, params, new(ziyan.SyncImageOption)); err != nil {
		logs.Errorf("sync tcloud ziyan image failed, err: %v, opt: %v, rid: %s", err, params, kt.Rid)
		return err
	}

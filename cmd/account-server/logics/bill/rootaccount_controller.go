...
func (rac *RootAccountController) runBillSummaryLoop(kt *kit.Kit) {
	wait.JitterUntil(
		func() error {
			err := rac.syncBillSummary(kt.NewSubKit())
			if err != nil {
		...
		converter.PtrToVal(cc.AccountServer().Controller.RootAccountSummarySyncDuration),
		0.5,
		true,
		kt.Ctx,
	)
...
func (rac *RootAccountController) runMonthTaskLoop(kt *kit.Kit) {
	wait.JitterUntil(
		...
		converter.PtrToVal(cc.AccountServer().Controller.MonthTaskSyncDuration),
		0.5,
		true,
		kt.Ctx,
	)

...
func upsertCmdbHosts[T corecvm.Extension](svc *cvmSvc, kt *kit.Kit, vendor enumor.Vendor, models []*cvm.Table,
	...
	for _, model := range models {
		if model.BkBizID == constant.UnassignedBiz || vendor == enumor.TCloudZiyan || model.Vendor == enumor.Other {
...
func deleteCmdbHosts(svc *cvmSvc, kt *kit.Kit, models []cvm.Table) error {
	...
	for _, one := range models {
		if one.BkBizID == constant.UnassignedBiz || one.Vendor == enumor.TCloudZiyan || one.Vendor == enumor.Other {
			continue
		}

...
func (svc *service) DeleteReturnPlanTicket(cts *rest.Contexts) (interface{}, error) {
	...
	for {
		listResp, err := svc.dao.ReturnPlanTicket().List(cts.Kit, listOpt)
		if err != nil {
			logs.Errorf("delete list return plan ticket failed, err: %v, rid: %s", err, cts.Kit.Rid)
			return nil, fmt.Errorf("delete list return plan ticket failed, err: %w", err)
		}
	...
	_, err := svc.dao.Txn().AutoTxn(cts.Kit, func(txn *sqlx.Tx, opt *orm.TxnOption) (interface{}, error) {
		for _, batchIDs := range slice.Split(delIDs, constant.BatchOperationMaxLimit) {
			...
			if err := svc.dao.ReturnPlanTicket().DeleteWithTx(cts.Kit, txn, delFilter); err != nil {
				return nil, err
			}
		}
		return nil, nil
	...
	if err != nil {
		logs.Errorf("delete return plan ticket failed, err: %v, rid: %s", err, cts.Kit.Rid)
		return nil, err
	}
